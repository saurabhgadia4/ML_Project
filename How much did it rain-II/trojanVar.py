...
if __name__=="__main__":
    ...
    # infile = 'ensemble_data\\train_cleaned.csv'
    ...
    stat_file = 'ensemble_data\\var'+r_int+'.txt'
    fobj = open(stat_file, 'w')
    
    # comp_mat = preprocess(infile, pps_mth, drop_list=drop_arr)
    fmat_file = 'C:\Users\saura\Desktop\ML_Project\\variance\\train_normalized_var.csv'
    flabel_file = 'C:\Users\saura\Desktop\ML_Project\\variance\\train_normalized_var_labels.csv'

    fmat = np.loadtxt(fmat_file, delimiter=',')
    print 'Done reading fmat'
    label = np.loadtxt(flabel_file, delimiter=',')
    print 'Done reading flabel'
    comp_mat = np.column_stack((fmat, label))
